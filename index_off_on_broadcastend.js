...
function mentionHtml(tg) {
  ...
  const id = tg?.id;
  const uname = tg?.username ? ` (@${escHtml(tg.username)})` : "";
  if (!id) return `<b>${escHtml(fullName)}</b>${uname}`;
  return `<a href="tg://user?id=${id}">${escHtml(fullName)}</a>${uname}`;
}
...
function userDocLabelHtml(u) {
  ...
  if (!u?.userId) return `<b>${escHtml(fullName)}</b>${uname}`;
  return `<a href="tg://user?id=${u.userId}">${escHtml(fullName)}</a>${uname}`;
}

function userDocLabelText(u) {
  if (!u) return "User";
  const fullName = fullNameFromUserDoc(u);
  return u?.username ? `${fullName} (@${u.username})` : fullName;
}
...
function challengeUserLabelHtml(userId, name, username) {
  ...
  if (!userId) return `<b>${safeName}</b>${uname}`;
  return `<a href="tg://user?id=${userId}">${safeName}</a>${uname}`;
}
...
function isMaintenanceBypassCommand(text = "") {
  ...
  return /^\/(on|off|broadcastend)(@\w+)?\b/.test(s);
}
...
function getUserVipRate(u) {
  return u?.isVip ? 0.90 : 0.50;
}

function spinSlotOutcomeForUser(u) {
  const wantsWin = Math.random() < getUserVipRate(u);
  ...
  for (let i = 0; i < 80; i++) {
    const a = weightedPick(SLOT.reels[0]);
    const b = weightedPick(SLOT.reels[1]);
    const c = weightedPick(SLOT.reels[2]);
    if (calcMultiplier(a, b, c) <= 0) return [a, b, c];
  }
