...
const USE_WEBHOOK = Boolean(PUBLIC_URL && WEBHOOK_SECRET);
...
function fullNameFromUserDoc(u) {
...
function getBotUsernamePlain() {
  return String(BOT_INFO?.username || "").replace(/^@/, "").trim();
}

function dmStartUrl(payload = "welcome") {
  const uname = getBotUsernamePlain();
  return uname ? `https://t.me/${uname}?start=${encodeURIComponent(payload)}` : null;
}

async function hasUserStartedBot(userId) {
  if (!userId) return false;
  const u = await users.findOne({ userId }, { projection: { startedBot: 1 } });
  return !!u?.startedBot;
}

function dmStartKeyboard(payload = "welcome") {
  const url = dmStartUrl(payload);
  if (!url) return undefined;
  return {
    inline_keyboard: [[{ text: "🤖 Start Bot in DM", url }]],
  };
}

function userDocLabelHtml(u) {
  if (!u) return `<b>User</b>`;
  const fullName = fullNameFromUserDoc(u);
  if (!u?.userId) return `<b>${escHtml(fullName)}</b>`;
  return `<a href="tg://user?id=${u.userId}">${escHtml(fullName)}</a>`;
}

function userDocLabelText(u) {
  if (!u) return "User";
  return fullNameFromUserDoc(u);
}

function challengeUserLabelHtml(userId, name, username) {
  const safeName = escHtml(name || username || "Player");
  if (!userId) return `<b>${safeName}</b>`;
...
async function ensureUser(tgUser) {
  ...
  const updated = extractUpdatedDoc(res);
...
bot.use(async (ctx, next) => {
  try {
    ...
    const isCommandEvent = ctx.updateType === "callback_query" || isCommandLikeText(text);
    if (!isCommandEvent) return next();

    const started = await hasUserStartedBot(ctx.from.id);
    if (started) return next();

    const msg = `🤖 <b>Bot ကိုအသုံးပြုဖို့ DM မှာ /start အရင်လုပ်ပေးပါ။</b>
━━━━━━━━━━━━
အောက်က button ကိုနှိပ်ပြီး bot private chat ထဲမှာ Start လုပ်ပြီးမှ ပြန်သုံးပါ။`;
    const replyMarkup = dmStartKeyboard("from_group");

    if (ctx.updateType === "callback_query") {
      try {
        await ctx.answerCbQuery("DM မှာ bot ကို /start အရင်လုပ်ပေးပါ။", { show_alert: true });
      } catch (_) {}
      if (ctx.chat?.id) {
        try {
    ...
    console.error("dm start middleware error:", e);
  }
  return next();
});
...
bot.use(async (ctx, next) => {
  try {
    ...
    const isCommandEvent = ctx.updateType === "callback_query" || isCommandLikeText(text);
    if (!isCommandEvent) return next();
    if (isMaintenanceBypassCommand(text)) return next();

    if (treasury?.maintenanceMode) {
      const msg = "🛠️ <b>Bot ပြုပြင်နေပါတယ်</b>\n━━━━━━━━━━━━\nခေတ္တစောင့်ဆိုင်းပေးပါ။";
      if (ctx.updateType === "callback_query") {
        try {
          await ctx.answerCbQuery("Bot ပြုပြင်နေပါတယ်။ ခေတ္တစောင့်ဆိုင်းပေးပါ။", { show_alert: true });
        } catch (_) {}
        return;
      }
...
async function ensureTreasury() {
  ...
  if (exist) {
    ...
    const fixedMaintenance = typeof exist.maintenanceMode === "boolean" ? exist.maintenanceMode : false;
    const fixedVipWinRate = Number.isFinite(Number(exist.vipWinRate)) ? Math.max(0, Math.min(100, Math.floor(Number(exist.vipWinRate)))) : 90;

    const needFix =
      fixedTotal !== exist.totalSupply ||
      fixedOwner !== exist.ownerBalance ||
      fixedOwnerId !== exist.ownerUserId ||
      fixedMaintenance !== exist.maintenanceMode ||
      fixedVipWinRate !== exist.vipWinRate;
      ...
      await configCol.updateOne(
        { key: "treasury" },
        {
          $set: {
            ...
            maintenanceMode: fixedMaintenance,
            vipWinRate: fixedVipWinRate,
            updatedAt: new Date(),
          },
        }
      );
...
function normalizeVipWinRate(value, fallback = 90) {
  const n = Number(value);
  if (!Number.isFinite(n)) return fallback;
  return Math.max(0, Math.min(100, Math.floor(n)));
}

function vipWinRateChance(vipWinRate) {
  return normalizeVipWinRate(vipWinRate, 90) / 100;
}
...
function isMaintenanceBypassCommand(text = "") {
  const s = String(text || "").trim().toLowerCase();
  return /^\/(on|off|broadcastend|setvipwr|vipwr|ping|status)(@\w+)?\b/.test(s);
}
...
function spinSlotOutcomeNormal() {
  return [
...
function spinSlotOutcomeVip(vipWinRate = 90) {
  if (Math.random() < vipWinRateChance(vipWinRate)) {
  ...
  return spinSlotOutcomeNormal();
}

function spinSlotOutcomeForUser(u, vipWinRate = 90) {
  return u?.isVip ? spinSlotOutcomeVip(vipWinRate) : spinSlotOutcomeNormal();
}

function decideDiceDuelValues(userA, userB, vipWinRate = 90) {
  let d1 = randInt(1, 6);
  let d2 = randInt(1, 6);

  const vipA = !!userA?.isVip;
  const vipB = !!userB?.isVip;
  const vipChance = vipWinRateChance(vipWinRate);
...
function drawShanHandsForUsers(userA, userB, vipWinRate = 90) {
  const tryOnce = () => {
  ...
  if (vipB && !vipA && Math.random() < vipChance) {
    for (let i = 0; i < 120; i++) {
      const out = tryOnce();
...
function parseVipTarget(text) {
  const parts = String(text || "").trim().split(/\s+/).filter(Boolean);
  if (parts.length === 1) return { mode: "reply", target: null };

  const rawTarget = String(parts[1] || "").trim();
  if (!rawTarget) return { mode: "reply", target: null };
  if (rawTarget.startsWith("@") && rawTarget.length > 1) {
    return { mode: "explicit", target: { type: "username", value: rawTarget.slice(1).toLowerCase() } };
  }
  if (/^\d+$/.test(rawTarget)) {
    return { mode: "explicit", target: { type: "userId", value: parseInt(rawTarget, 10) } };
  }
...
bot.command("addvip", async (ctx) => {
  ...
  const { mode, target } = parseVipTarget(ctx.message?.text || "");
  const r = await resolveTargetFromCtx(ctx, mode, target);
  if (!r.ok) {
    return replyHTML(
      ctx,
      "👤 VIP ပေးမယ့်သူကို Reply လုပ်ပါ သို့ <code>/addvip @username</code> / <code>/addvip userId</code> သုံးပါ။"
    );
  }
...
bot.command("removevip", async (ctx) => {
  ...
  if (!r.ok) {
    return replyHTML(
      ctx,
      "👤 VIP ဖြုတ်မယ့်သူကို Reply လုပ်ပါ သို့ <code>/removevip @username</code> / <code>/removevip userId</code> သုံးပါ။"
    );
  }
...
bot.command("setvipwr", async (ctx) => {
  ...
  if (parts.length < 2) {
    const current = normalizeVipWinRate(t?.vipWinRate, 90);
    return replyHTML(
      ctx,
      `⚙️ <b>Set VIP Win Rate</b>
━━━━━━━━━━━━━━━━
...
Applies to: <b>Slot / Dice / Shan</b>`
  ...
  const rate = normalizeVipWinRate(parts[1], NaN);
  if (!Number.isFinite(rate)) {
    return replyHTML(ctx, "⚠️ Usage: <code>/setvipwr 0-100</code>");
  }

  await configCol.updateOne(
    { key: "treasury" },
    { $set: { vipWinRate: rate, updatedAt: new Date() } }
  );

  return replyHTML(
    ctx,
    `✅ <b>VIP Win Rate Updated</b>
━━━━━━━━━━━━━━━━
New Rate: <b>${rate}%</b>
...
bot.command("ping", async (ctx) => {
  const started = Date.now();
  const dbPing = await getDbPingMs();
  ...
  return replyHTML(
    ...
    `🏓 <b>PING</b>
━━━━━━━━━━━━━━━━
⚡ Bot: <b>${Date.now() - started} ms</b>
🗄 DB: <b>${dbPing == null ? "N/A" : `${dbPing} ms`}</b>
⏱ Uptime: <b>${escHtml(uptime)}</b>
💾 Memory: <b>${rssMb} MB</b> RSS / <b>${heapMb} MB</b> Heap
🕒 Yangon: <b>${escHtml(formatYangon(new Date()))}</b>`
  );
});
...
bot.command("status", async (ctx) => {
  ...
  const started = Date.now();
  const [dbPing, userCount, groupCount, vipCount, openDiceCount, openShanCount] = await Promise.all([
    getDbPingMs(),
...
🎲 Open Dice: <b>${fmt(openDiceCount)}</b>
🀄 Open Shan: <b>${fmt(openShanCount)}</b>
📣 Broadcast: <b>${t?.broadcastRunning ? "RUNNING" : "IDLE"}</b>
🛠 Maintenance: <b>${t?.maintenanceMode ? "ON" : "OFF"}</b>
🎯 VIP WR: <b>${normalizeVipWinRate(t?.vipWinRate, 90)}%</b>`
  );
});
...
bot.command("viplist", async (ctx) => {
  ...
  try {
    const vipUsers = await users
      .find({ isVip: true })
      .sort({ updatedAt: -1, createdAt: -1, userId: 1 })
      .limit(100)
      .toArray();

    if (!vipUsers.length) {
      return replyHTML(ctx, `📭 <b>VIP List Empty</b>
━━━━━━━━━━━━━━━━
လက်ရှိ VIP user မရှိသေးပါ။`);
    }

    const lines = vipUsers.map((u, i) => {
      const label = userDocLabelHtml(u);
      return `${i + 1}. ${label} — <code>${u.userId}</code>`;
    });

    return replyHTML(ctx, `🌟 <b>VIP Users List</b>
━━━━━━━━━━━━━━━━
${lines.join("\n")}`);
  } catch (e) {
    console.error("viplist error:", e);
    return replyHTML(ctx, "⚠️ VIP list ထုတ်ရာတွင် အမှားအယွင်းရှိပါသည်။");
  }
});
...
━━━━━━━━━━━━━`
  ...
  if (currentBroadcast && !currentBroadcast.cancelled) {
    return replyHTML(ctx, "⚠️ Broadcast တစ်ခု လက်ရှိ run နေပါတယ်။ ရပ်ချင်ရင် <code>/broadcastend</code> သုံးပါ။");
  }

  const runId = `${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  const progress = await replyHTML(
    ctx,
    `📣 Broadcasting…
Target: users + groups
Status: <b>Running</b>
Run ID: <code>${runId}</code>`
  ...
  currentBroadcast = {
    ...
    progressMessageId: progress?.message_id || null,
    startedAt: Date.now(),
  };

  await configCol.updateOne(
    { key: "treasury" },
    { $set: { broadcastRunning: true, broadcastRunId: runId, updatedAt: new Date() } }
  );

  const seen = new Set();
  const targets = [];

  const userDocs = await users.find({}, { projection: { userId: 1 } }).toArray();
  for (const u of userDocs) {
    const id = Number(u?.userId);
    if (!Number.isFinite(id)) continue;
    if (seen.has(id)) continue;
    seen.add(id);
    targets.push({ chatId: id, kind: "user" });
  }

  const groupDocs = await groupsCol.find({ approvalStatus: "approved" }, { projection: { groupId: 1 } }).toArray();
  for (const g of groupDocs) {
    const id = Number(g?.groupId);
    if (!Number.isFinite(id)) continue;
    if (seen.has(id)) continue;
    seen.add(id);
    targets.push({ chatId: id, kind: "group" });
  }

  let ok = 0, fail = 0, skipped = 0;
  let userSent = 0, groupSent = 0;
  let stopped = false;

  for (let i = 0; i < targets.length; i++) {
    if (!currentBroadcast || currentBroadcast.id !== runId || currentBroadcast.cancelled) {
      stopped = true;
      break;
    }
    ...
    try {
      await safeTelegram(
        () =>
          bot.telegram.sendMessage(target.chatId, `📣 <b>BIKA Broadcast</b>
━━━━━━━━━━━━━━
${escHtml(text)}`, {
            parse_mode: "HTML",
            disable_web_page_preview: true,
      ...
      ok++;
      if (target.kind === "group") groupSent++; else userSent++;
    } catch (e) {
      const msg = String(e?.message || e || "").toLowerCase();
      if (msg.includes("chat not found") || msg.includes("forbidden") || msg.includes("blocked") || msg.includes("kicked") || msg.includes("migrated")) {
        skipped++;
      } else {
        fail++;
      }
    ...
    if ((i + 1) % 25 === 0 && currentBroadcast?.progressMessageId) {
      await editHTML(
        ctx,
        ctx.chat?.id,
        currentBroadcast.progressMessageId,
        `📣 <b>Broadcast Progress</b>
━━━━━━━━━━━━
Run ID: <code>${runId}</code>
Processed: <b>${i + 1}</b> / <b>${targets.length}</b>
Users sent: <b>${userSent}</b>
Groups sent: <b>${groupSent}</b>
Skipped: <b>${skipped}</b>
Failed: <b>${fail}</b>
Status: <b>Running</b>`
      );
  ...
  const progressMessageId = currentBroadcast?.progressMessageId;
  currentBroadcast = null;
  await configCol.updateOne(
    { key: "treasury" },
    { $set: { broadcastRunning: false, broadcastRunId: null, updatedAt: new Date() } }
  );

  if (progressMessageId) {
    try {
      await safeTelegram(() => bot.telegram.deleteMessage(ctx.chat.id, progressMessageId));
    } catch (_) {}
  }

  if (stopped) {
    return replyHTML(ctx, `🛑 Broadcast stopped.
• Sent: <b>${ok}</b>
• Users: <b>${userSent}</b>
• Groups: <b>${groupSent}</b>
• Skipped: <b>${skipped}</b>
• Failed: <b>${fail}</b>`);
  }
...
bot.command("broadcastend", async (ctx) => {
  ...
  if (!currentBroadcast || currentBroadcast.cancelled) {
    ...
    return replyHTML(ctx, "ℹ️ လက်ရှိ run နေတဲ့ broadcast မရှိပါ။");
  }

  currentBroadcast.cancelled = true;
  const ownerChatId = currentBroadcast.ownerChatId;
  const progressMessageId = currentBroadcast.progressMessageId;

  if (ownerChatId && progressMessageId) {
    try {
      await safeTelegram(() => bot.telegram.deleteMessage(ownerChatId, progressMessageId));
    } catch (_) {}
  }
  ...
  return replyHTML(ctx, "🛑 လက်ရှိ broadcast ကို ရပ်တန့်ပြီး clear လုပ်ပြီးပါပြီ။");
});
...
bot.command("gift", async (ctx) => {
  ...
  if (replyFrom?.id) {
    ...
    toUserId = toU.userId;
    toLabelHtml = userDocLabelHtml(toU);
  }
...
async function resolveTargetFromCtx(ctx, mode, target) {
  ...
  if (mode === "explicit" && target) {
    if (target.type === "username") {
      ...
      return { ok: true, userId: u.userId, labelHtml: userDocLabelHtml(u) };
    }
    if (target.type === "userId") {
      ...
      const found = await users.findOne({ userId: target.value });
      return { ok: true, userId: target.value, labelHtml: found ? userDocLabelHtml(found) : `<a href="tg://user?id=${target.value}">${target.value}</a>` };
    }
  }
...
async function runSlotSpinAnimated(ctx, bet) {
  ...
  const [slotUser, slotTreasury] = await Promise.all([
    ensureUser(ctx.from),
    ensureTreasury(),
  ]);
  ...
  try {
    ...
    const [finalA, finalB, finalC] = spinSlotOutcomeForUser(slotUser, slotTreasury?.vipWinRate ?? 90);
    ...
    const frames = [
      { a: randomSymbolFromReel(SLOT.reels[0]), b: randomSymbolFromReel(SLOT.reels[1]), c: randomSymbolFromReel(SLOT.reels[2]), note: "rolling…", vibe: "spin", delay: 140 },
      { a: finalA, b: randomSymbolFromReel(SLOT.reels[1]), c: randomSymbolFromReel(SLOT.reels[2]), note: "locking…", vibe: "spin", delay: 160 },
      { a: finalA, b: finalB, c: finalC, note: "result!", vibe: isJackpot ? "jackpot" : win ? "glow" : "lose", delay: 180 },
    ];

    if (SLOT.animationEnabled && messageId) {
...
bot.on("text", async (ctx, next) => {
  ...
  if (s.mode === "await_target") {
    ...
    if (text.startsWith("@") && text.length > 1) {
      ...
      targetLabel = userDocLabelText(u) || ("@" + uname);
    } else if (/^\d+$/.test(text)) {
...
function diceChallengeText(challenger, target, bet) {
  const challengerLabel = mentionHtml(challenger);
  const targetLabel = mentionHtml(target);
...
bot.hears(/^\.(dice)\s+(\d+)\s*$/i, async (ctx) => {
  ...
  activeDiceChallenges.set(challengeId, {
    ...
    challengerName: fullNameFromTg(ctx.from),
    challengerUsername: ctx.from.username || null,
    targetUserId: replyFrom.id,
    targetName: fullNameFromTg(replyFrom),
...
Bet process error.`);
        ...
        const challengerLabel = challengeUserLabelHtml(c.challengerId, c.challengerName, c.challengerUsername);
        const opponentLabel = challengeUserLabelHtml(c.opponentId, c.targetName, c.targetUsername);
...
Rolling dice...`,
        ...
        const t = await ensureTreasury();
        const { d1, d2 } = decideDiceDuelValues(challengerLatest, opponentLatest, t?.vipWinRate ?? 90);
...
━━━━━━━━━━━━
` +
              `စိန်ခေါ်သူ: ${challengerLabel} → <b>${d1}</b>
` +
              `လက်ခံသူ: ${opponentLabel} → <b>${d2}</b>
` +
              `━━━━━━━━━━━━
` +
...
Bet process error.`);
        ...
        const opponentLatest = await getUser(c.opponentId);
        const t = await ensureTreasury();
        const shanRound = drawShanHandsForUsers(challengerLatest, opponentLatest, t?.vipWinRate ?? 90);
...
(async () => {
  ...
  const webhookPath = WEBHOOK_SECRET ? `/telegraf/${WEBHOOK_SECRET}` : null;
  const webhookUrl = USE_WEBHOOK && webhookPath ? `${PUBLIC_URL}${webhookPath}` : null;
