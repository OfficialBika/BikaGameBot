...
function spinSlotOutcomeForUser(u, vipWinRate = 90) {
  if (u?.isVip && Math.random() < vipWinRateChance(vipWinRate)) {
...
bot.command("broadcast", async (ctx) => {
  ...
  const progress = await replyHTML(ctx, `📣 Broadcasting…\nTarget: users collection\nStatus: <b>Running</b>\nRun ID: <code>${runId}</code>`);
  currentBroadcast = {
    id: runId,
    cancelled: false,
...
async function runSlotSpinAnimated(ctx, bet) {
  ...
  try {
    ...
    const treasury = await ensureTreasury();
    const [finalA, finalB, finalC] = spinSlotOutcomeForUser(slotUser, treasury?.vipWinRate ?? 90);
