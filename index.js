  ...
  if (data.startsWith("DICE:")) {
    ...
    if (action === "ACCEPT") {
      ...
      if (toNum(challenger?.balance) < c.bet) {
        ...
        return ctx.editMessageText(`⚠️ <b>Challenge Failed</b>\n━━━━━━━━━━━━\nစိန်ခေါ်သူ balance မလုံလောက်ပါ။`, { parse_mode: "HTML", disable_web_page_preview: true });
      }
      ...
      if (toNum(opponent?.balance) < c.bet) {
        ...
        return replyHTML(ctx, `❌ <b>လက်ကျန်ငွေ မလုံလောက်ပါ</b>\n━━━━━━━━━━━━\nBet: <b>${fmt(c.bet)}</b> ${COIN}\nYour Balance: <b>${fmt(opponent?.balance)}</b> ${COIN}\nNeed More: <b>${fmt(lack)}</b> ${COIN}`, { reply_to_message_id: c.msgId });
      }
  ...
  if (data.startsWith("SHAN:")) {
    ...
    if (action === "ACCEPT") {
      ...
      try {
        ...
        c.status = "DONE";
        activeShanChallenges.delete(challengeId);
        return ctx.editMessageText(`⚠️ <b>Shan Duel Error</b>\n━━━━━━━━━━━━━━━━\nPayout error ဖြစ်လို့ refund ပြန်ပေးလိုက်ပါတယ်။`, { parse_mode: "HTML", disable_web_page_preview: true });
      }
