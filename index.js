...
bot.command("viplist", async (ctx) => {
  ...
  try {
    const list = await users
      .find({ isVip: true }, { projection: { userId: 1, username: 1, firstName: 1, lastName: 1, updatedAt: 1 } })
      .sort({ updatedAt: -1, createdAt: -1 })
      .limit(100)
      .toArray();

    if (!list.length) {
      return replyHTML(ctx, `🌟 <b>VIP List</b>
━━━━━━━━━━━━
No VIP users yet.`);
    }
    ...
    const lines = list.map((u, i) => {
      const label = u.username
        ? `@${escHtml(u.username)}`
        : `<a href="tg://user?id=${u.userId}">${escHtml(u.firstName || "User")}</a>`;
...
━━━━━━━━━━━━━`
  ...
  const msgHtml = `📣 <b>BIKA Broadcast</b>
━━━━━━━━━━━━━━
${escHtml(text)}`;
  const targets = [];
  const seen = new Set();

  const userList = await users.find({}, { projection: { userId: 1 } }).toArray();
  for (const u of userList) {
    ...
    seen.add(key);
    targets.push({ id, kind: "user" });
  }

  const groupList = await groupsCol.find({ approvalStatus: "approved" }, { projection: { groupId: 1 } }).toArray();
  for (const g of groupList) {
    ...
    if (seen.has(key)) continue;
    seen.add(key);
    targets.push({ id, kind: "group" });
  }

  if (!targets.length) {
    return replyHTML(ctx, "📣 Broadcast target မရှိသေးပါ။");
  }

  let okUsers = 0, okGroups = 0, failUsers = 0, failGroups = 0;
  await replyHTML(ctx, `📣 Broadcasting…
Users: <b>${fmt(userList.length)}</b>
Groups: <b>${fmt(groupList.length)}</b>
Unique total: <b>${fmt(targets.length)}</b>`);

  for (const target of targets) {
    try {
      await safeTelegram(() =>
        bot.telegram.sendMessage(target.id, msgHtml, {
          parse_mode: "HTML",
          disable_web_page_preview: true,
        }),
        { maxRetries: 3 }
      ...
      if (target.kind === "user") okUsers++;
      else okGroups++;
    } catch (e) {
      if (target.kind === "user") failUsers++;
      else failGroups++;
    }
    await sleep(35);
  }
...
━━━━━━━━━━━━
` +
      `👤 Users success: <b>${fmt(okUsers)}</b>
` +
      `👥 Groups success: <b>${fmt(okGroups)}</b>
` +
      `❌ Users failed: <b>${fmt(failUsers)}</b>
` +
      `❌ Groups failed: <b>${fmt(failGroups)}</b>
` +
      `📦 Unique total tried: <b>${fmt(targets.length)}</b>`
  );
});
