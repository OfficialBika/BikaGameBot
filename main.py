...
BAL_TEMPLATE = TEMPLATES_DIR / "bal.png"
TOP10_TEMPLATE = TEMPLATES_DIR / "top10.png"
...
def trim_text(text: Any, limit: int = 20) -> str:
...
def template_rank_title(balance: int) -> str:
    b = int(balance or 0)
    if b <= 0:
        return "Starter"
    if b <= 500:
...
def _font_candidates(bold: bool = False) -> List[Path]:
    preferred = [
        FONTS_DIR / ("NotoSansMyanmar-Bold.ttf" if bold else "NotoSansMyanmar-Regular.ttf"),
...
def draw_glow_text(draw, xy: Tuple[int, int], text: str, font, fill=(255, 248, 220), glow=(255, 170, 50), stroke=1):
    x, y = xy
    for dx, dy in [(-2,0),(2,0),(0,-2),(0,2),(-1,-1),(1,1),(-1,1),(1,-1)]:
        draw.text((x+dx, y+dy), text, font=font, fill=glow)
    draw.text(xy, text, font=font, fill=fill, stroke_width=stroke, stroke_fill=(90, 40, 0))


def build_balance_card_image(fullname: str, username: Optional[str], balance: int) -> Optional[BufferedInputFile]:
    if not PIL_AVAILABLE or not BAL_TEMPLATE.exists():
        return None
    try:
        img = Image.open(BAL_TEMPLATE).convert("RGBA")
        draw = ImageDraw.Draw(img)

        w, h = img.size
        name_font = get_font(max(26, w // 34), True)
        small_font = get_font(max(22, w // 42), True)
        time_font = get_font(max(20, w // 50), False)

        fullname_val = trim_text(fullname or "Unknown", 20)
        username_val = trim_text(username_text(username), 20)
        balance_val = fmt(balance)
        rank_val = template_rank_title(balance)
        time_val = format_yangon()

        name_xy = (int(w * 0.20), int(h * 0.23))
        username_xy = (int(w * 0.24), int(h * 0.34))
        balance_xy = (int(w * 0.22), int(h * 0.45))
        rank_xy = (int(w * 0.18), int(h * 0.56))
        time_xy = (int(w * 0.17), int(h * 0.67))

        draw_glow_text(draw, name_xy, fullname_val, name_font)
        draw_glow_text(draw, username_xy, username_val, name_font)
        draw_glow_text(draw, balance_xy, balance_val, name_font)
        draw_glow_text(draw, rank_xy, rank_val, name_font)
        draw_glow_text(draw, time_xy, time_val, time_font, fill=(255, 244, 210), glow=(255, 150, 40))

        buf = BytesIO()
        img.save(buf, format="PNG")
        return BufferedInputFile(buf.getvalue(), filename="bal_card.png")
    except Exception:
        return None


def build_top10_card_image(rows: List[Tuple[str, int]]) -> Optional[BufferedInputFile]:
    if not PIL_AVAILABLE or not TOP10_TEMPLATE.exists():
        return None
    try:
        img = Image.open(TOP10_TEMPLATE).convert("RGBA")
        draw = ImageDraw.Draw(img)
        w, h = img.size

        name_font = get_font(max(24, w // 34), True)
        bal_font = get_font(max(24, w // 34), True)

        start_y = int(h * 0.224)
        row_gap = int(h * 0.0845)
        name_x = int(w * 0.33)
        bal_x = int(w * 0.74)
        ...
        for i in range(10):
            ...
            if i < len(rows):
                name, bal = rows[i]
                name_val = trim_text(name or "Unknown", 18)
                bal_val = fmt(bal)
            else:
                name_val = "-"
                bal_val = "-"
            draw_glow_text(draw, (name_x, y), name_val, name_font)
            draw_glow_text(draw, (bal_x, y), bal_val, bal_font)

        buf = BytesIO()
        img.save(buf, format="PNG")
        return BufferedInputFile(buf.getvalue(), filename="top10_card.png")
    except Exception:
        return None
...
async def cmd_balance(message: Message) -> None:
    if not await ensure_not_maintenance(message):
        return
    user = await ensure_user(message.from_user)
    await reply_html(message, f"💼 Balance: <b>{fmt(user.get('balance'))}</b> {COIN}")
...
async def cmd_top10(message: Message) -> None:
    ...
    for u in docs:
        rows.append((user_doc_label_text(u), to_num(u.get("balance"))))

    photo = build_top10_card_image(rows)
...
async def cmd_mybalance(message: Message) -> None:
    if not await ensure_not_maintenance(message):
        return
    if not is_group_message(message) and not (message.text or "").startswith("/bal"):
        await reply_html(message, "ℹ️ ဒီ command ကို group ထဲမှာပဲ သုံးနိုင်ပါတယ်။")
        return
    if is_group_message(message) and not await ensure_group_approved(message):
        return
    u = await ensure_user(message.from_user)
    bal = to_num(u.get("balance"))

    photo = build_balance_card_image(
        fullname=(message.from_user.full_name or u.get("fullName") or "Unknown"),
        username=message.from_user.username or u.get("username"),
        balance=bal,
    )
    if photo is not None:
        await message.answer_photo(photo)
...
async def cb_dice(cb: CallbackQuery) -> None:
    ...
    d1_msg = await bot.send_dice(c["chatId"], emoji=DiceEmoji.DICE, reply_to_message_id=c["msgId"])
    await asyncio.sleep(1.1)
    d2_msg = await bot.send_dice(c["chatId"], emoji=DiceEmoji.DICE, reply_to_message_id=c["msgId"])
